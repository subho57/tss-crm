...
NXINVOICE.toggleTaxDiscountButtons = function (value = 0) {
  ...
  //log
  NXINVOICE.log(
    "[billing] toggleTaxDiscountButtons() - based on billing value [bill_final_amount]: (" +
      bill_final_amount +
      ")"
  );
...
NXINVOICE.DOM.domState = function () {
  NXINVOICE.log(
    "[billing] state() - setting billing doma state- [payload]",
    NXINVOICE.DATA.INVOICE
  );
...
NXINVOICE.toggleTaxDom = function (tax_type = "") {
  NXINVOICE.log(
    "[billing] initialiseTaxPopover() - toggling tax popover - [tax_type]: " +
      tax_type
  );
...
NXINVOICE.toggleDiscountDom = function (bill_discount_type = "") {
  ...
  //log
  NXINVOICE.log(
    "[billing] toggleDiscountDom() setting popover state [current_discount_type]: (" +
      bill_discount_type +
      ")"
  );
...
NXINVOICE.toggleAdjustmentDom = function () {
  NXINVOICE.log(
    "[billing] toggleAdjustmentDom() setting adjustment popover state"
  );
...
NXINVOICE.DOM.updateAmountDue = function (bill_final_amount = 0) {
  NXINVOICE.log(
    "[billing] NXINVOICE.DOM.updateAmountDue() - updating amount due label [bill_final_amount]: (" +
      bill_final_amount +
      ")"
  );
  ...
  if (amount_due > 0) {
    ...
    amount_due_label
      .html(accounting.formatMoney(amount_due))
...
NXINVOICE.DOM.itemNewLine = function (data = {}) {
  NXINVOICE.log(
    "[billing] NXINVOICE.DOM.itemNewLine() - cloning time line - [payload]"
  );
  NXINVOICE.log(data);
  ...
  var item_description =
    data.item_description != null ? data.item_description : " &$; ";
  ...
  var item_linked_type =
    data.item_linked_type != null ? data.item_linked_type : "";
  ...
  if (item_linked_type != "") {
    ...
    if ($("input[data-duplicate-check='" + check + "']").length > 0) {
      NXINVOICE.log(
        "[billing] NXINVOICE.DOM.itemNewLine() - the item being added is a duplicate. Will skip (" +
          check +
          ")"
      );
  ...
  if (item_category == "Static") {
    ...
    lineitem.find(".static-class").css({ display: "none" });
  ...
  lineitem
    .find(".js_item_product")
    .attr("name", "js_item_product[" + uniqueid + "]");
  lineitem
    .find(".js_item_variation")
    .attr("name", "js_item_variation[" + uniqueid + "]");
  lineitem
    .find(".js_item_description")
    .attr("name", "js_item_description[" + uniqueid + "]");
  ...
  lineitem
    .find(".js_item_quantity")
    .attr("name", "js_item_quantity[" + uniqueid + "]");
  lineitem.find(".js_item_unit").attr("name", "js_item_unit[" + uniqueid + "]");
  lineitem
    .find(".js_item_unit_height")
    .attr("name", "js_item_unit_height[" + uniqueid + "]");
  lineitem
    .find(".js_item_unit_width")
    .attr("name", "js_item_unit_width[" + uniqueid + "]");
  lineitem.find(".js_item_rate").attr("name", "js_item_rate[" + uniqueid + "]");
  lineitem
    .find(".js_item_total")
    .attr("name", "js_item_total[" + uniqueid + "]");
  lineitem
    .find(".js_linetax_rate")
    .attr("name", "js_linetax_rate[" + uniqueid + "]");
  lineitem
    .find(".js_item_linked_type")
    .attr("name", "js_item_linked_type[" + uniqueid + "]");
  lineitem
    .find(".js_item_linked_id")
    .attr("name", "js_item_linked_id[" + uniqueid + "]");
  lineitem
    .find(".js_item_linked_type")
    .attr("data-duplicate-check", item_linked_type + "|" + item_linked_id); //used for tracking duplicates
...
NXINVOICE.DOM.timeNewLine = function (data = {}) {
  NXINVOICE.log(
    "[billing] NXINVOICE.DOM.timeNewLine() - cloning time line - [payload]"
  );
  NXINVOICE.log(data);
  ...
  var item_description =
    data.item_description != null ? data.item_description : "";
  var item_unit =
    data.item_unit != null ? data.item_unit : NXLANG.invoice_time_unit;
  var item_hours = data.item_hours != null ? data.item_hours : "";
  var item_minutes = data.item_minutes != null ? data.item_minutes : "";
  var item_rate = data.item_rate != null ? data.item_rate : "";
  var item_total = data.item_total != null ? data.item_total : "";
  var item_linked_id = data.item_linked_id != null ? data.item_linked_id : "";
  var item_timers_list =
    data.item_timers_list != null ? data.item_timers_list : "";
  ...
  lineitem
    .find(".js_item_hours")
    .attr("name", "js_item_hours[" + uniqueid + "]");
  lineitem
    .find(".js_item_minutes")
    .attr("name", "js_item_minutes[" + uniqueid + "]");
  lineitem.find(".js_item_unit").attr("name", "js_item_unit[" + uniqueid + "]");
  lineitem.find(".js_item_rate").attr("name", "js_item_rate[" + uniqueid + "]");
  lineitem
    .find(".js_item_total")
  ...
  lineitem
    ...
    .attr("name", "js_item_linked_id[" + uniqueid + "]");
  lineitem
    .find(".js_item_timers_list")
    .attr("name", "js_item_timers_list[" + uniqueid + "]");
  lineitem.find(".js_item_type").attr("name", "js_item_type[" + uniqueid + "]");
...
NXINVOICE.CALC.recalculateLines = function () {
  NXINVOICE.log(
    "[billing] recalculateLines() - validating and recalculating each line item"
  );

  //(1) ------------------------------ find each line item --------------------------------
  $("#billing-items-container")
    .find(".billing-line-item")
    .each(function () {
      NXINVOICE.log(
        "[billing] recalculateLines() - found a line item. Now validating and calculating it"
      );
      ...
      NXINVOICE.log(
        "[billing] recalculateLines() - [quantity]: (" +
          quantity +
          ") - [rate]: (" +
          rate +
          ") - [linetax]: (" +
       ...
       * [02-04-2021]
       * --------------------------------------------------*/
      if (unit_height == "" || unit_height == null || unit_height < 0) {
      ...
      if (unit_width == "" || unit_width == null || unit_width < 0) {
      ...
      if (type == "plain") {
        ...
        if (quantity > 0 && rate > 0) {
          ...
          } else if (unit_width > 0 && unit_height > 0 && job > 0) {
            var sqmm = (unit_width * unit_height) / 1000000;
            linetotal = quantity * rate * sqmm;
          }
          linetotal = linetotal + (Number(job) / 10 - 0.1) * linetotal;
          ...
          NXINVOICE.log(
            "[billing] reclaculateBill() - line item is valid. [line item total]: " +
              linetotal
          );
        } else {
          NXINVOICE.log(
            "[billing] reclaculateBill() - line item is invalid and is skipped"
          );
          total.val("");
        }
      }
...
NXINVOICE.DOM.deleteLine = function (self) {
  NXINVOICE.log("[billing] NXINVOICE.DOM.deleteLine() - deleting line item");
...
NXINVOICE.DOM.addSelectedProductItems = function (self) {
  NXINVOICE.log(
    "[billing] NXINVOICE.DOM.addSelectedProductItems() - adding items selected in add items modal"
  );
...
NXINVOICE.DOM.addSelectedExpense = function (self) {
  NXINVOICE.log(
    "[billing] NXINVOICE.DOM.addSelectedExpense() - adding expenses selected in add expenses modal"
  );
...
NXINVOICE.DOM.addSelectedTimebilling = function (self) {
  NXINVOICE.log(
    "[billing] NXINVOICE.DOM.addSelectedTimebilling() - adding hours selected in add time billing modal"
  );
...
NXINVOICE.updateTax = function () {
  ...
  NXINVOICE.log(
    "[billing] updateTaxType() - updating tax type [type]: (" + tax_type + ")"
  );
  ...
  $("#billing-tax-popover-summary-info")
    .find(".js_summary_tax_rate")
    .each(function () {
      //mark as selected
      if ($(this).is(":checked")) {
        //get uniqie tax id
...
NXINVOICE.updateAdjustment = function () {
  NXINVOICE.log(
    "[billing] updateAdjustment() - updating bill adjustment amount"
  );
  ...
  if (bill_adjustment_amount < 0) {
    ...
    $("#billing-adjustment-container-amount").html(
      "-" + accounting.formatMoney(bill_adjustment_amount)
    );
  } else {
    $("#billing-adjustment-container-amount").html(
      accounting.formatMoney(bill_adjustment_amount)
...
NXINVOICE.removeAdjustment = function () {
  //log
  NXINVOICE.log(
    "[billing] removeAdjustment() - updating bill adjustment amount to zero"
  );
...
NXINVOICE.updateDiscount = function () {
  ...
  if (discount_type == "percentage") {
    NXINVOICE.log(
      "[billing] updateDiscountType() - [percentage] ",
      Number($("#js_bill_discount_percentage").val())
    );
    if (
      Number($("#js_bill_discount_percentage").val()) > 100 ||
      Number($("#bill_discount_type").val()) <= 0
    ) {
      //error message
      NX.notification({
        type: "error",
  ...
  if (discount_type == "fixed") {
    NXINVOICE.log(
      "[billing] updateDiscountType() - [fixed] ",
      Number($("#js_bill_discount_amount").val())
    );
  ...
  if (discount_type == "fixed") {
    ...
    $("#bill_discount_percentage").val(0.0);
  ...
  if (discount_type == "percentage") {
    ...
    $("#dom-billing-discount-type").html(
      "(" + $("#js_bill_discount_percentage").val() + "%)"
    );
    $("#bill_discount_percentage").val($("#js_bill_discount_percentage").val());
    $("#bill_discount_amount").val(0.0);
  }
...
NXINVOICE.CALC.reclaculateBill = function (self) {
  ...
  //default tax rate
  var bill_tax_total_percentage = 0.0;

  //amount before an deductions
  var bill_subtotal = 0.0;

  //total discount amount
  var bill_discount_amount = 0.0;

  //total tax amount
  var bill_tax_total_amount = 0.0;

  //total before tax
  var bill_amount_before_tax = 0.0;

  //bill sum
  var bill_final_amount = 0.0;
  ...
  //(1) ----------------------- SUM UP LINE ITEMS -------------------------------------
  NXINVOICE.log(
    "[billing] reclaculateBill() - summing up all line items - started"
  );
  $("#billing-items-container")
    .find(".billing-line-item")
    .each(function () {
      ...
      if (typeof linetotal == "number") {
        NXINVOICE.log(
          "[billing] reclaculateBill() - valid line item found [total]: (" +
            linetotal +
            ")"
        );
  ...
  NXINVOICE.log(
    "[billing] reclaculateBill() - updating subtotal [subtotal]: (" +
      bill_subtotal +
      ") "
  );
  ...
  NXINVOICE.log(
    "[billing] reclaculateBill() - calculating discounts [bill_discount_type]: (" +
      bill_discount_type +
  ...
  if (bill_discount_type == "percentage") {
    ...
    //log
    NXINVOICE.log(
      "[billing] reclaculateBill() - discount is percentage based. [bill_discount_amount]: (" +
        bill_discount_amount +
        ")"
    );
  }
  ...
  if (bill_discount_type == "fixed") {
    //log
    NXINVOICE.log(
      "[billing] reclaculateBill() - discount is fixed. [bill_discount_amount]: (" +
        bill_discount_amount +
        ")"
    );
  }
  ...
  if (bill_discount_amount > 0) {
    ...
    //discount amount
    $("#billing-sums-discount").html(
      accounting.formatMoney(bill_discount_amount)
    );
    //log
    NXINVOICE.log(
      "[billing] reclaculateBill() - there is a discount - setting DOM  and [bill_amount_before_tax]: (" +
        bill_amount_before_tax +
        ")"
    );
    //set amount before tas
    $("#billing-sums-before-tax").html(
      accounting.formatMoney(bill_amount_before_tax)
    );
  } else {
    //hide subtotal
    ...
    //log
    NXINVOICE.log(
      "[billing] reclaculateBill() - there is no discount - [bill_amount_before_tax]: (" +
        bill_amount_before_tax +
        ")"
    );
  }
  ...
  if ($("#bill_tax_type").val() == "summary") {
    ...
    $("#bill_logic_taxes")
      ...
      .each(function () {
        ...
        //create table row
        tax_row +=
          '<tr class="billing-sums-tax-container" id="billing-sums-tax-container-' +
          uniqueid +
          '">' +
          "<td>" +
          taxname +
          ...
          "%)</span></td>" +
          "<td>" +
          accounting.formatMoney(tax_amount) +
          "</td></tr>";
    ...
    if (bill_tax_total_percentage > 0) {
      //log
      NXINVOICE.log(
        "[billing] reclaculateBill() - this bill has [summary] based tax [tax_percentage]: (" +
          bill_tax_total_percentage +
          "%)"
      );
      //tax calculation
      bill_tax_total_amount =
        (bill_amount_before_tax * bill_tax_total_percentage) / 100;
      ...
      //log
      NXINVOICE.log(
        "[billing] reclaculateBill() - the total tax on this bill is [total_tax_amount]: (" +
          bill_tax_total_amount +
          ")"
      );
    } else {
      NXINVOICE.log(
        "[billing] reclaculateBill() - this bill does not have any applicable tax"
      );
  ...
  bill_final_amount =
    bill_subtotal - bill_discount_amount + bill_tax_total_amount;
  NXINVOICE.log(
    "[billing] reclaculateBill() - total amount before adjustment. [bill_final_amount]: (" +
      bill_final_amount +
      ")"
  );
  ...
  //adjustment
  NXINVOICE.log(
    "[billing] reclaculateBill() - accounting for the adjustment. [bill_adjustment_amount]: (" +
  ...
  NXINVOICE.log(
    "[billing] reclaculateBill() - bill claculation finished [final amount]: (" +
      bill_final_amount +
      ")"
  );
...
NXINVOICE.CALC.validateLines = function (self) {
  ...
  //log
  NXINVOICE.log(
    "[billing] validateLines() - (" + count_bill_error + ") error found"
  );
...
NXINVOICE.DOM.revalidateItem = function (self) {
  ...
  if (
    self.hasClass("js_item_rate") ||
    self.hasClass("js_item_quantity") ||
    self.hasClass("js_item_hours") ||
    self.hasClass("js_item_minutes") ||
    self.hasClass("js_item_unit_height") ||
    self.hasClass("js_item_unit_width")
  ) {
